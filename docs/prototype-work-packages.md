...
| WP2 | Relay conformance probe and routed-path measurement | WP0 | ✅ Complete — public contract (amended 2026-08-30 to the in-band session profile), browser probe, in-memory adapter and the deterministic suite, plus the routed acceptance of 2026-08-30: five pinned-browser sessions (three sequential, two concurrent with distinct assigned addresses) over a routed path, the in-band terminal refusal observed, zero-length datagrams carried both ways, and the validated merged record committed with a 1,024-byte datagram maximum and a 768-byte conservative inner floor for WP6 |
