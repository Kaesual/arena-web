...
| WP4 | One-map offline browser arena with bots | WP1, WP3 | Implemented, witnessed acceptance pending — the first witnessed attempt (2026-08-30) caught an intermittent WebGL renderer defect the automated checks had never looked at pixels for; it is mitigated by the reviewed `r_vertexLight` workaround, guarded by a new near-white regression check, and its root-cause hunt is deferred until after WP6 (see the WP4 evidence document); the witnessed round restarts |
